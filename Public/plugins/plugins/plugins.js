...
Plugins.notify = function(opts){
	var Message = function(opts){
	    ...
	    this.element = $(['<div class="alert e-notify-message" role="alert">',
	                      this._opts.msg,
	                '</div>'].join('')).data("notifyMessage", this);
	    if($('.e-notify-'+this._opts.pos).length==0)$('<div class="e-notify e-notify-'+this._opts.pos+'"></div>').appendTo('body');
	    if (this._opts.status) {
	        this.element.addClass('alert-'+this._opts.status);
	    }
	    $(this.element).appendTo('.e-notify-'+this._opts.pos);
	}
	$.extend(Message.prototype, {
	    ...
	    close: function(){
	    	...
	    	this.element.animate({"opacity":0, "margin-top": -1* this.element.outerHeight(), "margin-bottom":0}, function(){
	    		$this.element.remove();
	    		if(!$('.e-notify'+$this._opts.pos).children().length){
	    			$('.e-notify'+$this._opts.pos).hide();
	    		}
	        });
	    }
	});
...
Plugins.window = function(opts){
    ...
    this.element.find('.modal-dialog').css('margin-left',(E.pageWidth()-this._opts.width)/2);
...
Plugins.Modal = function(opts){
    ...
    if(this._opts.height)this.element.find('.modal-dialog').height(this._opts.height);
    this.element.find('.modal-dialog').css('margin-left',(E.pageWidth()-this._opts.width)/2);
    this.element.find('.modal-dialog').css('margin-top',(E.pageHeight()-this._opts.height)/4);
    $('#'+this._opts.id).delegate('[data-dismiss="modal"]', 'click.dismiss.modal', function(){
		Plugins.closeWindow();
	});
    this.element.modal('show');
...
$.fn.TabPanel = function(options){
	...
	var t = this;
	$(t).find('.e-tab-nav li').click(function(){
		$(this).addClass("on").siblings().removeClass();
		var index = $(this).index();
		$(t).find('.e-tab-content .e-tab-item').eq(index).show().siblings().hide();
		if(opts.callback)opts.callback(index);
	});
	$(t).find('.e-tab-nav li').eq(opts.tab).click();
}